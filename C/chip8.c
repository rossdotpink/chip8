...
void loop(chip8_t *system) {
  while (true) {
    ...
    log_debug(
        "Current Instruction 0x%04X @ PC: 0x%03X \
      1st Word: 0x%04X \
      2nd Word: 0x%04X \
      3rd Word: 0x%04X \
      4th Word: 0x%04X",
        instruction, system->program_counter, get_first_nibble(&instruction),
        get_second_nibble(&instruction), get_third_nibble(&instruction),
...
uint16_t fetch(chip8_t *system) {
  ...
  // log_debug("Current Instruction %04X @ PC: 0x%03X", instruction,
  // system->program_counter); log_trace("0x%03x\t%02x%02x",
  // (system->program_counter), system->memory[MEM_OFFSET],
...
void cls(chip8_t *system) {
  for (int i = 0; i < 32; i++) system->frame_buffer[i] = 0;
  // return OK;
}
